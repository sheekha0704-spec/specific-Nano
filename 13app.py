...
def load_and_prep():
    ...
    for col in cat_cols:
        # Fix for TypeError: fillna and convert to string
    ...
    for col in cat_cols:
        le = LabelEncoder()
        # Fit on whole df to prevent 'unseen label' ValueError
        le.fit(df[col])
...
# --- STEP 5: RESULTS ---
    ...
    with t1:
        ...
        # Diagram logic (Visual marker only)
    ...
    with t2:
        ...
        rel = np.clip(kh * np.sqrt(time_points), 0, 100)
        st.plotly_chart(px.line(x=time_points, y=rel, title="Higuchi Release Profile"), use_container_width=True)

    with t3:
        grid = 10; o_rng = np.linspace(5, 40, grid); s_rng = np.linspace(10, 50, grid); z = np.zeros((grid, grid))
        for i, o in enumerate(o_rng):
            for j, s in enumerate(s_rng): z[i,j] = stab_model.predict(idx)[0]
