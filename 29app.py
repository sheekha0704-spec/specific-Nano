...
def load_and_clean_data(uploaded_file=None):
    # Industrial flexibility: Use uploaded file if present, else fallback to default
...
# Persistent file handling for industrial CSV upload
uploaded_file = st.sidebar.file_uploader("Industrial Work: Browse CSV", type="csv")
...
if df is not None:
    # --- STEP 1: SOURCING (WITH SMILES & Uploader) ---
    if nav == "Step 1: Sourcing":
        ...
        with c_top2:
            smiles = st.text_input("Industrial Input: Enter Drug SMILES", placeholder="e.g. CC1=C(C(=CC=C1)C)NC(=O)CN(CC)CC")
    ...
    # --- STEP 4: AI PREDICTION (WITH SHAP DESCRIPTION) ---
        ...
        try:
            if 'drug' not in st.session_state or 'f_o' not in st.session_state:
                ...
                **What is SHAP (SHapley Additive exPlanations)?**
                SHAP is a method used to explain the output of the machine learning model. It breaks down the prediction to show how much each component (Drug, Oil, Surfactant) contributed to the final result.
                
                **How to interpret the Waterfall Plot below:**
                * **Baseline (E[f(X)]):** The average predicted droplet size across all formulations in the dataset.
                * **Red Bars (Positive Values):** These ingredients are **increasing** the droplet size. For example, a heavy oil phase might push the size higher.
                * **Blue Bars (Negative Values):** These ingredients are **decreasing** the droplet size. This indicates a successful "nano" effect by that component.
                * **Final Prediction (f(x)):** The resulting size for your current formulation selection.
                """)
    ...
    st.error("Please ensure a valid CSV file is available or uploaded.")
