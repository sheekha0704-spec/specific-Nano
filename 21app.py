...
def load_and_prep(uploaded_file=None):
    # Fix for FileNotFound: Check if file is uploaded or exists locally
...
with st.sidebar:
    ...
    with st.expander("📖 Parameter Justifications"):
        st.markdown("<div class='justification'><b>Size & PDI:</b> Relates to droplet surface area.</div>", unsafe_allow_html=True)
        st.markdown("<div class='justification'><b>Zeta Potential:</b> Determines shelf-life stability.</div>", unsafe_allow_html=True)
...
if step_choice == "Step 1: Drug & Oil":
    ...
    if le_dict:
        ...
        if st.button("Finalize and Prediction"):
    ...
    if le_dict:
        ...
        with cp1:
            ...
            # Image tags must be outside or commented to prevent SyntaxError
