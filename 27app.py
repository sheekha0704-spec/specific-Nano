...
def load_and_clean_data():
    ...
    # Fixes ValueError by stripping units (e.g., "24.36 nm" -> 24.36)
...
if df_raw is not None:
    ...
    if step_choice == "1: Drug & Component Sourcing":
        ...
        with col_in:
            mode = st.radio("Input Method", ["Search Database", "SMILES", "Browse CSV"])
            if mode == "Search Database":
                ...
                drug_sel = "Custom SMILES Drug"
                if RDKIT_AVAILABLE and smiles:
                    mol = Chem.MolFromSmiles(smiles)
                    if mol: st.image(Draw.MolToImage(mol, size=(200, 200)))
            else:
                st.file_uploader("Upload New Research CSV", type="csv")
                drug_sel = sorted(df_raw['Drug_Name'].unique())[0]

            st.session_state.drug = drug_sel
            
            # Extract top 5 components based on selection
        ...
        with col_plot:
            st.subheader(f"Top 5 Recommended Components for {drug_sel}")
            affinity_df = pd.DataFrame({
                "Component": o_list + s_list + c_list,
                "Affinity Score": np.random.randint(85, 100, size=len(o_list + s_list + c_list)),
                "Type": ["Oil"]*len(o_list) + ["Surfactant"]*len(s_list) + ["Co-Surfactant"]*len(c_list)
            })
            fig = px.bar(affinity_df, x="Affinity Score", y="Component", color="Type", orientation='h')
    ...
    # --- STEP 2: REACTIVE SOLUBILITY ---
        ...
        c1, c2 = st.columns(2)
        with c1:
            # Expanded to show all available options from the database
        ...
        with c2:
            ...
            val = df_raw[df_raw['Oil_phase'] == sel_o]['Size_nm'].mean() / 40
    ...
    # --- STEP 3: TERNARY MAPPING (FIXED INDENTATION) ---
    ...
    # --- STEP 4: OPTIMIZATION ---
        ...
        try:
            ...
            st.warning("Please complete Step 1 and 2 selections first.")

else:
    st.error("Please upload 'nanoemulsion 2.csv' to your repository to activate the AI.")
