...
# --- PAGE CONFIG ---
st.set_page_config(page_title="NanoPredict AI v19.0 - Proprietary API Suite", layout="wide")
...
st.markdown("""
    ...
    .warning-box { background: #fff5f5; border-left: 5px solid #e53e3e; padding: 15px; border-radius: 8px; color: #c53030; margin-top: 10px; }
...
if st.session_state.step_val == "Step 1: Chemical Setup":
    ...
    with c1:
        ...
        if mode == "Database API":
            ...
            drug_name = "Proprietary API"

        analysis = analyze_structure(smiles)
        if analysis:
            st.session_state.logp = analysis['logp']
            st.session_state.mw = analysis['mw']
            st.session_state.fg = analysis['fg']
            st.session_state.smiles = smiles
            st.session_state.drug = drug_name
            
            st.write(f"**LogP:** {analysis['logp']:.2f}")
            st.write(f"**MW:** {analysis['mw']:.2f} g/mol")
            st.write(f"**Groups:** {', '.join(analysis['fg']) if analysis['fg'] else 'None Detected'}")
            
            # OIL SELECTION BASED ON HEATMAP
            st.write("---")
            oil_selection = st.selectbox("Select Oil Phase (Suggested top of list)", list(OIL_HSP.keys()))
            if st.button("Confirm Setup →"):
                st.session_state.oil = oil_selection
    ...
    with c2:
        if analysis:
            ...
            for name, hsp in OIL_HSP.items():
                dist = np.sqrt(4*(analysis['hsp'][0]-hsp[0])**2 + (analysis['hsp'][1]-hsp[1])**2 + (analysis['hsp'][2]-hsp[2])**2)
            ...
            st.success(f"AI structural match found: {match} ({score*100:.1f}%)")

# --- STEP 2: CONCENTRATIONS ---
elif st.session_state.step_val == "Step 2: Concentrations":
    st.header("Step 2: Loading & Solubility Limits")
    c1, c2 = st.columns(2)
    with c1:
    ...
    with c2:
        ...
        st.metric("Max Loading Capacity", f"{max_load:.2f} mg/mL")

# --- STEP 3: SCREENING ---
elif st.session_state.step_val == "Step 3: AI Screening":
    st.header("Step 3: Component AI Screening")
    # Screen surfactants that worked best with the chosen oil in the database
    ...
    with c2:
        st.markdown('<div class="rec-box"><b>HLB Data</b>', unsafe_allow_html=True)
...
# --- STEP 4: SELECTION ---
    ...
    c1, c2 = st.columns(2)
    with c1:
        st.session_state.s_final = st.selectbox("Final Surfactant", sorted(df_raw['Surfactant'].unique()))
        st.session_state.cs_final = st.selectbox("Final Co-Surfactant", sorted(df_raw['Co-surfactant'].unique()))
        if st.button("Execute Final AI Run →"): go_to_step("Step 5: Results")
    with c2:
        st.markdown(f"""<div class="summary-table"><h4>📋 Review Summary</h4>
...
# --- STEP 5: RESULTS ---
    ...
    # IMPORTANT: Use the match_drug to get valid predictions for New Drugs
    ...
    t1, t2 = st.tabs(["Phase Design & Stability", "Release Kinetics"])
    with t1:
        c1, c2 = st.columns(2)
        with c1:
    ...
    with t2:
        ...
        # Prediction formula using LogP from Step 1
