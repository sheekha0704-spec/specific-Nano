...
def find_best_match(smiles):
    ...
    # Search within labels already known by the AI
    known_drugs = le_dict['Drug_Name'].classes_
    for drug in known_drugs:
        if drug == "Not Specified" or drug == "New API": continue
        try:
            # For speed, we use a simple lookup. In production, you'd cache these SMILES.
...
with st.sidebar:
    ...
    try: current_idx = nav.index(st.session_state.step_val)
...
if st.session_state.step_val == "Step 1: Chemical Setup":
    ...
    with c1:
        ...
        if analysis:
            ...
            # Perform similarity match immediately
    ...
    with c2:
        if analysis:
            ...
            if st.session_state.match_drug:
                st.success(f"Structural Match Found: **{st.session_state.match_drug}** ({score*100:.1f}% Similarity)")
...
# --- STEP 5: RESULTS (FIXED FOR UNKNOWN COMPOUNDS) ---
elif st.session_state.step_val == "Step 5: Results":
    st.header("Step 5: AI Performance Suite")
    
    # CORE FIX: If the drug is "New API", we use the "match_drug" name for the AI label transform
    # This prevents the "unseen labels" error.
    if st.session_state.drug == "New API":
        predict_label = st.session_state.match_drug
        st.info(f"💡 Predicting for unknown compound based on structural similarity to **{predict_label}**.")
    else:
        predict_label = st.session_state.drug

    try:
        inputs = [[le_dict['Drug_Name'].transform([predict_label])[0],
        ...
        t1, t2 = st.tabs(["Stability & Phase Diagram", "Release Profile"])
        with t1:
            c1, c2 = st.columns(2)
            with c1:
